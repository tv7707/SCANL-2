...
<!-- Key for all comments with sterotypes selected by id of class
-->
<xsl:key name="allslist" match="src:class/src:block/src:comment[contains(., '@stereotype')]"
  use="generate-id(../..)"/>
...
<xsl:key name="expslist" match="src:class/src:block/src:comment[contains(., '@stereotype')]"
  use="str:split(str:replace(concat(' ', normalize-space(substring-before(substring-after(., '@stereotype'),'*/'))), ' ', concat(' ', generate-id(current()/../..), '-')))"/>

<func:function name="src:methods">
  <func:result select="count(current()/src:block/src:function)"/>
</func:function>
...
<func:function name="src:creational">
  <func:result select="src:count('constructor') + src:count('factory')"/>
</func:function>
...
<xsl:template match="src:class" mode="table">
  ...
  <!-- Stereotype comments -->
  <xsl:variable name="sstereotype_count" select="count(key('allslist', generate-id(.)))"/>
